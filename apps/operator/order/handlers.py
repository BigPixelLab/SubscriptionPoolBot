...
async def order_handler(message: Message or CallbackQuery = None, command: CommandObject = None):
    ...
    order_models.Order.mark_as_taken(order.id, message.from_user.id)
    order.processed_by = message.from_user.id
